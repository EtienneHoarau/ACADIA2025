...
noise_factor = 0.5
X_train_noised = X_train+noise_factor*np.random.normal(loc=0.0,scale=1.0,size = X_train.shape)
X_valid_noised = X_valid+noise_factor*np.random.normal(loc=0.0,scale=1.0,size = X_valid.shape)

X_train_noised = np.clip(X_train_noised,0.,1.)
X_valid_noised = np.clip(X_valid_noised,0.,1.)

print(X_train[1].shape)
print(X_train_noised[1].shape)
print(X_valid[1].shape)
print(X_valid_noised[1].shape)

# Chemin de fichier de sauvegarde formaliser
checkpoint_filepath = 'conv_aeDebruiteur_model'
...
if not os.path.exists(checkpoint_filepath):
    ...
    # Entraîner le modèle
    conv_ae.fit(X_train_noised, X_train, epochs=20, validation_data=(X_valid_noised, X_valid),shuffle='true')
    ...
    conv_ae.save("./ae/"+checkpoint_filepath , save_format='tf')
    ...
    conv_encoder = tf.keras.models.load_model("./ae/"+checkpoint_filepath)
...
plot_reconstructions(conv_ae, X_valid_noised)
plt.show()
